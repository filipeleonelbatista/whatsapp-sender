  ...
  return (
    <DrawerComponent title="Blog">
      <Typography variant='h5'>
        Blog Dicas do Zapzap.
      </Typography>
      <Typography variant='body1'>
        Os conteúdos do blog são exclusivos para os assinantes e ficam disponíveis 1 mês após no blog do site.
      </Typography>
        ...
        <Box sx={{ width: '100%' }}>
          {
            posts.length === 0 && (
              <Box>
                <Typography variant="h5">
                  Os conteúdos estão sendo preparados para você.
                </Typography>
                <Typography variant="body1">
                  Em breva você verá conteúdos de valor aqui!
                </Typography>
              </Box>
            )
          }
            ...
            posts.map(post => (
              <Box key={post.id} sx={{
                ...
                <Box sx={{ mt: 1, mb: 1, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                  ...
                  <Typography variant="caption">
                    <b>Categorias:</b> {
                      post.categorias?.map(cat => <span style={{ marginRight: '8px' }} key={cat.id}>{cat.nome}</span>)
                    }
                  </Typography>
                </Box>
